...
train.to_csv("your path/input_data/de_train.tsv", sep="\t", index=False)
val.to_csv("your path/input_data/de_dev.tsv", sep="\t", index=False)
test.to_csv("your path/input_data/de_test.tsv", sep="\t", index=False)
df.to_csv("your path/input_data/de_data.tsv", sep="\t", index=False)

FILE_PATH: str = Path(__file__).resolve()
INPUT_DATA_PATH = FILE_PATH.parent / "input_data"  # Changed to relative path
...
class BackTranslationBaseline:
    ...
    def process_dataset(
        ...
        for idx, row in data.iterrows():
            lang = row['lang']
            if lang in backtranslations and idx < len(backtranslations[lang]):
                neutral_sentences.append(backtranslations[lang][idx])
            else:
...
def parse_args():
    ...
    parser.add_argument(
        "--input_path",
        type=str,
        default=Path(INPUT_DATA_PATH, "de_test.tsv"),
        help="Path to input TSV file containing toxic sentences",
    )
    parser.add_argument(
        "--output_path",
        type=str,
        default=Path(OUTPUT_DATA_PATH, "de_baseline_backtranslation_test.tsv"),
